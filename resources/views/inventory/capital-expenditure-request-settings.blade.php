...
@section('padded_content')
    <h1 style="text-align: center">Capital Expenditure Request Settings</h1>
    <br>
    <div class="row">
        <div class="col-sm-12">
...
app.controller('content-controller', function($scope,$http, $sce, $window) {
    ...
    $scope.show_recipients = function(url_string) {
        url_string = (typeof url_string !== 'undefined') && url_string !== "" ? url_string : route('api.inventory.capital-expenditure-request.recipient.list').url();
        $scope.items = {};
        $scope.loading = true;
        $scope.pages = "";
        $http({
    ...
    $scope.add_recipient = function() {
        $scope.formerrors = {};
            ...
            $http({
                method: 'POST',
                url: route('api.inventory.capital-expenditure-request.recipient.store').url(),
    ...
    $scope.edit_recipient = function(item) {
        ...
        $http({
            method: 'PATCH',
            url: route('api.inventory.capital-expenditure-request.recipient.update',[item.id]).url(),
            data: $.param(item)
        }).then(function(response) {
            $scope.submit = false;
        }, function(rejection) {
    ...
    $scope.delete_confirm = function(item) {
        alertify.confirm(
            'DELETE RECIPIENT',
            item.user.name+' can no longer receive capital expenditure request email notification, continue?',
            function(){
    ...
    $scope.delete_recipient = function(item) {
        ...
        $http({
            method: 'DELETE',
            url: route('api.inventory.capital-expenditure-request.recipient.destroy',[id]).url(),
        }).then(function(response) {
            $scope.show_recipients();
    ...
    $scope.update_footer_settings = function() {
        $http({
            method: 'PATCH',
            url: route('api.inventory.capital-expenditure-request.footer.update').url(),
