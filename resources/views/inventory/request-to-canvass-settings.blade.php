...
@section('padded_content')
    <h1 style="text-align: center">Request to Canvass Settings</h1>
    <br>
    <div class="row">
        <div class="col-sm-12">
...
app.controller('content-controller', function($scope,$http, $sce, $window) {
    ...
    $scope.add_recipient = function() {
        $scope.formerrors = {};
            ...
            $http({
                method: 'POST',
                url: route('api.inventory.request-to-canvass.recipient.store').url(),
    ...
    $scope.edit_recipient = function(item) {
        ...
        $http({
            method: 'PATCH',
            url: route('api.inventory.request-to-canvass.recipient.update',[item.id]).url(),
            data: $.param(item)
        }).then(function(response) {
            $scope.submit = false;
        }, function(rejection) {
    ...
    $scope.delete_recipient = function(item) {
        ...
        $http({
            method: 'DELETE',
            url: route('api.inventory.request-to-canvass.recipient.destroy',[id]).url(),
        }).then(function(response) {
    ...
    $scope.update_footer_settings = function() {
        $http({
            method: 'PATCH',
            url: route('api.inventory.request-to-canvass.footer.update').url(),
