...
@endsection
@section('breadcrumb')
<div class="active section">Request to Canvasses</div>
<i class="right angle icon divider"></i>
<a class="section hideprint" href="{{route('inventory.request-to-canvass.create')}}" ng-if="user_data.privilege == 'admin' || user_data.permissions.can_add_request_to_canvasses">Create Request to Canvass</a>
<i class="divider" ng-if="user_data.privilege == 'admin' || user_data.permissions.can_add_request_to_canvasses">|</i>
<a class="section" href="{{route('inventory.request-to-canvass.settings')}}">Settings</a>
@endsection

@section('two_row_content')
    <h1 style="text-align: center">Request to Canvasses</h1>
    <br>
    <div class="row">
        <div class="col-sm-5">
            <div class="ui left icon input">
                <input type="text" placeholder="Search Request to Canvass Number" ng-model="searchString" ng-keyup="search()">
        ...
        <div class="col-sm-12">
            <div class="table-responsive">
                <table class="table table-hover">
                    <thead>
                        <tr>
                            <th style="text-align: center">RTC No.</th>
                    ...
                    <tbody ng-repeat="item in items" ng-cloak>
                        <tr>
                            <td style="text-align: center">
                                <a ng-href="@{{item.form}}" target="_blank">@{{item.request_to_canvass_number_formatted}}</a>
                            </td>
                            ...
                            <td style="text-align: center">
                                <div class="ui buttons">
                                    <button type="button" class="ui blue button" ng-click="edit_form(item)" ng-if="user_data.privilege == 'admin' || user_data.permissions.can_edit_request_to_canvasses"><span class="glyphicon glyphicon-edit"></span></button>
                                    <button type="button" class="ui red button" ng-click="delete_confirm(item)" ng-if="user_data.privilege == 'admin' || user_data.permissions.can_delete_request_to_canvasses"><span class="glyphicon glyphicon-trash"></span></button>
                                </div>
                            </td>
                        </tr>
                    </tbody>
...
app.controller('content-controller', function($scope,$http, $sce, $window) {
    ...
    $scope.show_items = function(url_string) {
        url_string = (typeof url_string !== 'undefined') && url_string !== "" ? url_string : route('api.inventory.request-to-canvass.list').url();
        $scope.items = {};
        $scope.loading = true;
        $scope.pages = "";
        $http({
    ...
    $scope.edit_form = function(item) {
        window.location = route('inventory.request-to-canvass.edit',[item.uuid]).url();
    }

    $scope.delete_confirm = function(item) {
        alertify.confirm(
            'DELETE REQUEST TO CANVASS',
            'Are you sure to delete this request to canvass. This action is irreversible?',
            function(){
    ...
    $scope.delete_form = function(id) {
        $http({
            method: 'DELETE',
            url: route('api.inventory.request-to-canvass.delete',[id]).url(),
        }).then(function(response) {
            $scope.show_items();
            $.notify('Request to Canvass has been deleted.');
        }, function(rejection) {
            if (rejection.status != 422) {
                request_error(rejection.status);
            } else if (rejection.status == 422) {
