...
@section('title', 'Purchase Order Email Notification')
...
@section('padded_content')
    <h1 style="text-align: center">Purchase Order Settings</h1>
    <br>
    <div class="row">
        <div class="col-sm-12">
...
app.controller('content-controller', function($scope,$http, $sce, $window) {
    ...
    $scope.show_recipients = function(url_string) {
        url_string = (typeof url_string !== 'undefined') && url_string !== "" ? url_string : route('api.inventory.purchase-order.recipient.list').url();
        $scope.items = {};
        $scope.loading = true;
        $scope.pages = "";
        $http({
    ...
    $scope.add_recipient = function() {
        $scope.formerrors = {};
            ...
            $http({
                method: 'POST',
                url: route('api.inventory.purchase-order.recipient.store').url(),
    ...
    $scope.edit_recipient = function(item) {
        ...
        $http({
            method: 'PATCH',
            url: route('api.inventory.purchase-order.recipient.update',[item.id]).url(),
            data: $.param(item)
        }).then(function(response) {
            $scope.submit = false;
        }, function(rejection) {
    ...
    $scope.delete_recipient = function(item) {
        ...
        $http({
            method: 'DELETE',
            url: route('api.inventory.purchase-order.recipient.destroy',[id]).url(),
        }).then(function(response) {
            $scope.show_recipients();
    ...
    $scope.update_footer_settings = function() {
        $http({
            method: 'PATCH',
            url: route('api.inventory.purchase-order.footer.update').url(),
