...
<a class="section hideprint" href="{{route('inventory.purchase-order.create')}}" ng-if="user_data.privilege == 'admin' || user_data.permissions.can_add_purchase_orders">Create Purchase Order</a>
<i class="divider" ng-if="user_data.privilege == 'admin' || user_data.permissions.can_add_purchase_orders">|</i>
    ...
    <div class="row">
        <div class="col-sm-5">
            <div class="ui left icon input">
                <input type="text" placeholder="Search Purchase Order Number" ng-model="searchString" ng-keyup="search()">
                ...
                <table class="table table-hover">
                    ...
                    <tbody ng-repeat="(index,item) in items" ng-cloak>
                        <tr>
                            ...
                            <td style="text-align: center">@{{item.purchase_order_date_formatted}}</td>
                            ...
                            <td style="text-align: center">
                                <div class="ui buttons">
                                    <button type="button" class="ui green button" ng-click="approve_confirm(item,index)" ng-if="!item.is_approved && (user_data.privilege == 'admin' || user_data.permissions.can_approve_purchase_orders)"><span class="glyphicon glyphicon-ok"></span></button>
                                    <button type="button" class="ui blue button" ng-click="edit_form(item)" ng-if="user_data.privilege == 'admin' || user_data.permissions.can_edit_purchase_orders"><span class="glyphicon glyphicon-edit"></span></button>
                                    <button type="button" class="ui red button" ng-click="delete_confirm(item)" ng-if="user_data.privilege == 'admin' || user_data.permissions.can_delete_purchase_orders"><span class="glyphicon glyphicon-trash"></span></button>
                                </div>
                            </td>
                        </tr>
                    </tbody>
...
app.controller('content-controller', function($scope,$http, $sce, $window) {
    ...
    $scope.show_items = function(url_string) {
        url_string = (typeof url_string !== 'undefined') && url_string !== "" ? url_string : route('api.inventory.purchase-order.list').url();
        $scope.items = {};
        $scope.loading = true;
        $scope.pages = "";
        $http({
    ...
    $scope.edit_form = function(item) {
        window.location = route('inventory.purchase-order.edit',[item.uuid]).url();
    }
    ...
    $scope.delete_form = function(id) {
        $http({
            method: 'DELETE',
            url: route('api.inventory.purchase-order.delete',[id]).url(),
        }).then(function(response) {
            $scope.show_items();
            $.notify('Purchase Order has been deleted.');
        }, function(rejection) {
            if (rejection.status != 422) {
                request_error(rejection.status);
            } else if (rejection.status == 422) {
